...
- 2026-08-23 — Fixed the promoted primary Swap action. Its associated form remains housed in the overflow component for submission plumbing, but an external primary submitter now follows the inline AJAX swap path; only a submitter physically inside the overflow sheet can select the legacy native-navigation fallback. Script-contract and browser interaction coverage protect the distinction.
- 2026-08-23 — Hardened the primary Swap fix after live diagnostics showed the generic submit listener firing without the AJAX swap listener completing. The hidden primary swap form no longer carries the overflow-action marker, and AJAX submit interception now runs in the capture phase before the generic loading listener; the primary action therefore cannot enter the legacy overflow/native-submit branch or be pre-empted by listener order.
- 2026-08-23 — Removed the local OTLP collector as an implicit development dependency. Development now keeps OTLP export disabled by default, eliminating repeated four-second `localhost:4318` connection failures when the optional observability stack is absent; operators can still opt in through `OBSERVABILITY__ENABLE_OTLP=true`, with the existing local endpoint retained.
- 2026-08-23 — Completed the live primary-meal Swap repair after browser runtime evidence exposed two stacked failures. The main bundle now imports the shared submit-button resolver before intercepting the form, so the request can no longer be cancelled by a `ReferenceError`; when a full seven-day plan exhausts unused breakfast or lunch templates, swap selection now follows the planner's existing two-occurrence limit instead of rejecting every in-plan candidate. Dinner uniqueness and seen-meal exclusions remain enforced. Service, script-contract, and real mobile-browser regressions cover the complete path.
