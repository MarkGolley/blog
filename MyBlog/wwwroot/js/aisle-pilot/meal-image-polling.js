...
if (isLoading) {
delete shell.dataset.mealImageUnavailable;
...
const markPendingImagesUnavailable = pollContext => {
if (!pollContext) return;
getPendingMealImageNames(pollContext).forEach(imageElements => imageElements.forEach(imageElement => {
const shell = resolveImageShell(imageElement);
if (!(shell instanceof HTMLElement)) return;
setMealImageLoadingState(imageElement, false);
shell.dataset.mealImageUnavailable = "true";
}));
};
...
markPendingImagesUnavailable(pollContext);
stop();
return;
}
pollAttempts += 1;
